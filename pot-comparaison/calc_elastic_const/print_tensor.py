...
with open(r'run_lammps_test/elastic/pot-ML-w-less-atoms-elastic/log.lammps','r') as logfile:
    txt = logfile.read()
...
lam = mean([c[0][1], c[0][2], c[1][2]])
mu = mean([c[i][i] for i in range (3,6)])
lam_2mu = mean([c[i][i] for i in range (0,3)])
young = mu*(3*lam + 2*mu)/(lam + mu)

print (lam,mu,lam_2mu, (lam + 2*mu))

print(young, (75*(3*53 + 2*75))/(75+53))
